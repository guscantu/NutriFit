...
BASE_URL = "http://127.0.0.1:5000/api/faixas_peso"

class FaixaPesoApp:
    def __init__(self, master):
        self.master = master
        master.title("Gerenciar Faixas de Peso")
        master.geometry("650x500")

        
        tk.Label(master, text="ID").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        tk.Label(master, text="Faixa Mínima (kg)").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        tk.Label(master, text="Faixa Máxima (kg)").grid(row=2, column=0, sticky="w", padx=10, pady=5)
    ...
    def cadastrar(self):
        try:
            ...
            if response.status_code == 201:
                messagebox.showinfo("Sucesso", "Faixa de peso cadastrada com sucesso!")
                self.limpar_campos()
                self.listar_todos()
            else:
                messagebox.showerror("Erro", response.json().get("message", "Erro ao cadastrar faixa de peso."))
    ...
    def buscar(self):
        ...
        if not faixa_id:
            messagebox.showwarning("Aviso", "Informe o ID da faixa de peso.")
            return
        try:
            ...
            if response.status_code == 200:
                ...
                messagebox.showwarning("Atenção", "Faixa de peso não encontrada.")
        except Exception as e:
            messagebox.showerror("Erro", str(e))
    ...
    def atualizar(self):
        ...
        try:
            ...
            if response.status_code == 200:
                messagebox.showinfo("Sucesso", "Faixa de peso atualizada com sucesso!")
                self.listar_todos()
            else:
                messagebox.showerror("Erro", response.json().get("message", "Erro ao atualizar faixa de peso."))
        except ValueError as e:
            messagebox.showwarning("Entrada inválida", str(e))
        except Exception as e:
            messagebox.showerror("Erro", str(e))
    ...
    def excluir(self):
        faixa_id = self.id_entry.get().strip()
        if not faixa_id:
            messagebox.showwarning("Aviso", "Informe o ID da faixa de peso para excluir.")
            return
        try:
            response = requests.delete(f"{BASE_URL}/{faixa_id}")
            if response.status_code == 204:
                messagebox.showinfo("Sucesso", "Faixa de peso excluída com sucesso!")
                self.limpar_campos()
                self.listar_todos()
            else:
                messagebox.showerror("Erro", response.json().get("message", "Erro ao excluir faixa de peso."))
        except Exception as e:
            messagebox.showerror("Erro", str(e))
    ...
    def listar_todos(self):
        try:
            ...
            if response.status_code == 200:
                ...
                if dados:
                    self.resultado.insert(tk.END, "Lista de faixas cadastradas:\n\n")
                    for item in dados:
                        self.resultado.insert(
                            tk.END,
                            f"ID: {item['id']} | Mín: {item['faixa_min']} kg | Máx: {item['faixa_max']} kg\n"
                        )
                else:
                    self.resultado.insert(tk.END, "Nenhuma faixa de peso cadastrada.")
                self.resultado.config(state=tk.DISABLED)
            else:
